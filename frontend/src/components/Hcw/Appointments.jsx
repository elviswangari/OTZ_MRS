...
const HcwAppointments = () => {
  const { visitData, loading } = useVisitData('hcw/appointments');
    ...
    <DashboardLayout role="hcw">
      <div className="flex flex-col gap-6">
          ...
          <div className="space-y-1">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbLink href="/hcw/appointments">Appointments</BreadcrumbLink>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
            ...
            <p className="text-muted-foreground">
              {activePatient 
                ? `Managing appointments for ${activePatient.firstName} ${activePatient.surname}`
                : "Manage and track patient follow-up visits."}
            </p>
          </div>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <CardTitle>{activePatient ? 'Patient Appointment History' : 'Upcoming Appointments'}</CardTitle>
            <CardDescription>
              {activePatient 
                ? `Showing all appointments for CCC: ${activePatient.cccNumber}`
                : "A list of scheduled visits for the facility."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
